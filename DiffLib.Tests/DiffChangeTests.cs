...
namespace DiffLib.Tests
{
    ...
    public class DiffChangeTests
    {
        ...
        {
            var ds = new DiffChange(equal, 0, 0, length1, length2);
        ...
        {
            var ds1 = new DiffChange(equal, 0, 0, length1, length2);
            var ds2 = new DiffChange(equal, 0, 0, length1, length2);
        ...
        {
            var ds1 = new DiffChange(equal, 0, 0, length1, length2);
            object ds2 = new DiffChange(equal, 0, 0, length1, length2);
        ...
        {
            Assert.Throws<ArgumentException>(() => new DiffChange(true, 0, 0, 1, 2));
        }

        [Test]
        public void Constructor_NegativeLength1_ThrowsArgumentOutOfRangeException()
        {
            Assert.Throws<ArgumentOutOfRangeException>(() => new DiffChange(false, 0, 0, -1, 1));
        }
